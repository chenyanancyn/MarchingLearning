...
def bagging_fit(data, n_estimators=1, max_depth=None, minsize=1, n_features=1):
    trees = []
    for i in range(n_estimators):
        sub_samples = bootstrap(data)
        # 建树
        tree = CART_Classifier.fit(sub_samples, max_depth, minsize, n_features)
        trees.append(tree)
...
def bagging_predict(trees, sample):
    pred_label = []
    for tree in trees:
        label = CART_Classifier.predict(tree, sample)
        pred_label.append(label)
    
    return max(pred_label, key=pred_label.count)



# -*- coding :UTF-8 -*-
"""CART分类树课件总结，第一种递归方法，不带group的"""
...
def get_split(dataSet, n_features=1):#求最优特征索引和最优二分标准
    class_values=list(set(row[-1] for row in dataSet))
    b_index,b_value,b_score,b_groups=999,999,999,None
	# features=list(range(0,len(dataSet[0])-1))
    # # 用于随机森林
    # sub_feature = []
    # for i in range(n_features):
    #     j = random.randrange(len(features))
    #     sub_feature.append(features[j])
    #     del(features[j])
...
def createTree(data,max_depth,minsize,depth, n_features):#递归建树
    labelList=[d[-1]  for d in data]
    if (len(set(labelList))==1) or len(data)<=minsize or depth>=max_depth:
        return toLeafNode(labelList)

    node=get_split(data, n_features)
    left,right=node[2][0],node[2][1]

    tree={'index':node[0],'value':node[1],'left':{},'right':{}}
	
    tree['left']=createTree(left,max_depth,minsize,depth+1, n_features)
    tree['right']=createTree(right,max_depth,minsize,depth+1, n_features)
    return tree	

def fit(train,max_depth=None,minsize=1, n_features=1):#有的递归是第一项区别对待的，同时方便设定参数,因此添加一个函数
	if max_depth==None:
		max_depth=999
	depth=0
	root=createTree(train,max_depth,minsize,depth, n_features)
	return root
