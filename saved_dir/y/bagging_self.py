...
def bagging(data_set, n_features=None, n_trees=5):
    if n_features is None:
        n_features = len(data_set) - 1
    trees = []
    for i in range(n_trees):
        sub_samples = bootstrap(data_set)
        tree = fit(sub_samples, n_features, max_depth=5)
        trees.append(tree)
...
# trees = bagging(train, n_trees=10)
# # print(tree)
# # 进行预测
...
# 3.每棵树都不进行后剪枝
# 4.最终随机森林的预测结果即为所有树模型预测结果的简单组合（分类决定）
# 如果k=d,那随机森和Bagging算法相同
# 如果k=1，那就是完全随机
# 一般k=d,k=log2(d), k=sqrt(d)
...
# 训练并建树
# tree = fit(train, max_depth=10, min_size=2)    # 当max_depth适度变小时，正确率会提高 ****   
trees = bagging(train, n_trees=10, n_features=n_features)
# print(tree)
# 进行预测
# pre = [predict(tree, d) for d in validation_data]
pre = [bagging_predict(trees, d) for d in validation_data]
