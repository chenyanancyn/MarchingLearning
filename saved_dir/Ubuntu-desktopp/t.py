...
# np.empty((s,d*f))
# [X_train, y_train] = read.read_and_decode('./data/train1.tfrecords', 4000)
# print(X_train.shape)
# print(y_train.shape)
...
# 第二层卷积：5×5×32卷积核64个 [3，3，32，64],h_conv2.shape=[-1, 20, 28, 64]
...
# flatten层，[-1, 5, 7, 96]->[-1, 7*7*64],即每个样本得到一个7*7*64维的样本
...
with tf.Session() as sess:
    ...
    for j in range(6):
        ...
        [X_train, y_train] = read.read_and_decode('./data/train%d.tfrecords' % (j+1), 4000)
        for i in range(3000):
            X_batch, y_batch = chooseone(X_train, y_train, 50)
            x_v, y_v = chooseone(X_val, y_val, 50)
            # X_batch, y_batch = mnist.train.next_batch(batch_size=50)
            if i % 100 == 0:
                train_accuracy = accuracy.eval(feed_dict={X_: x_v, y_: y_v, y_1: y_v[:, :11], y_2: y_v[:, 11:22], y_3: y_v[:, 22:33], y_4: y_v[:, 33:44], keep_prob: 1.0})
                print("step %d, training acc %g" % ((i+j*3000), train_accuracy))
                summary, train_accuracy1, train_accuracy2, train_accuracy3, train_accuracy4 = sess.run([merged,accuracy1, accuracy2, accuracy3, accuracy4], feed_dict={X_: x_v, y_: y_v, y_1: y_v[:, :11], y_2: y_v[:, 11:22], y_3: y_v[:, 22:33], y_4: y_v[:, 33:44], keep_prob: 1.0})
                ...
                # train_accuracy1 = accuracy1.eval(feed_dict={X_: x_v, y_: y_v, y_1: y_v[:, :11], y_2: y_v[:, 11:22], y_3: y_v[:, 22:33], y_4: y_v[:, 33:44], keep_prob: 1.0})
                ckptname = os.path.join(FLAGS.my_list, FLAGS.my_cnn)
                if train_accuracy > 0.86:
                    saver.save(sess, ckptname, global_step=i+j*3000)
                train_writer.add_summary(summary, i)
            train_step.run(feed_dict={X_: X_batch, y_: y_batch, y_1: y_batch[:, :11], y_2: y_batch[:, 11:22], y_3: y_batch[:, 22:33], y_4: y_batch[:, 33:44], keep_prob: 0.5})  
    
    
# 全部训练完了再做测试，batch_size=100
    for i in range(100): 
        X_batch, y_batch = chooseone(X_test, y_test, 200)
...
#     Y = np.zeros(4314)
#     X_batch, y_batch = chooseone(X_val, y_val, 4314)
#     Ytemp = y_conv.eval(feed_dict={X_: X_batch, keep_prob: 1.0})
#     for i in range(4314):
#         Y[i] = np.argmax(Ytemp[i])
# print(Y)
    # print("测试集正确率为 %g" % accuracy.eval(feed_dict={X_: X_batch, y_: y_batch, keep_prob: 1.0}))